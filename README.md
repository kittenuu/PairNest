...
只是「打开读一眼」的话，很多网站给不了什么——时间线、列表页、视频站的内容都要往下翻才出得来，
有的还没登录就先弹个框把页面盖住。所以小窗还有第二种用法：**一个一直停在那里的页面**。

手机上进「恋爱记忆 → 小窗 → 进去逛逛」，输个网址，画面就出现在页面里：

- 点画面上的哪里，就点网页里的哪里；
- 底下有上翻、下翻、返回、刷新；
- 想在网页的输入框里打字，先点那个框，再用底下的输入条填进去；
- 「实时」可以暂停，省流量；「关掉」把页面收起来，省服务器内存。

画面是轮询的截图（jpeg，手机上流量不大），不是视频流——**所以看得到页面，但看不了视频播放**。

### 登录：扫码永远你自己来

要看需要登录才给内容的网站，就在「进去逛逛」里把它打开，**扫码、密码、验证码都由你自己操作**。
登录态存在浏览器的资料目录里，挂了持久盘就能长期保留，办一次管很久，之后 AI 那边看到的
就是你的账号视角。

两条务必守住：

- **只登内容类账号**（视频、社区这种）。**支付类、银行类、手机厂商账号永远不要登**——
  这个浏览器是能被 AI 调用的，让它持有你的付款能力，风险和收益完全不成比例；
- 登录这一步始终是人在做，不要把密码交给任何自动流程。
...
停在那里接着看的（对应上面「进去逛逛」的同一个页面）：

- `page_open`：在常驻页面里打开网址；
- `page_look`：看这一页现在的样子——正文、滚到哪了、这一屏有哪些能操作的东西；
- `page_do`：翻页、点开某一条、在框里填字、按键、回上一页、重新加载；
- `page_shot`：把当前画面截下来。文字读不到内容时（视频站、图片站常见）用它；
- `page_done`：关掉，省内存。登录态不会丢。

能操作的元素是**带编号**列出来的，指认时给编号即可，不依赖页面的 class 或结构，
网站改版也不容易失效。
